...
void key_filter_init(void)
{
	...
	key_config.interrupt_mode = k_gpio_int_fall_edge;
	key_config.default_output = 1;
	...
	system_register_irq_handler(GPIO1_Combined_16_31_IRQn,
								(system_irq_handler_t)gpio1_16_31_irq_handler, NULL);
...
void key_filter_timer_init(unsigned int value)
{
	...
	/* 注册中断服务函数 */
	system_register_irq_handler(EPIT1_IRQn, (system_irq_handler_t)key_filter_timer_irq_handler, NULL);
}
...
{
	/* 重置定时器 */
	key_filter_timer_restart(66000000/100);
